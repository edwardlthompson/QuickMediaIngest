...
HUMAN_RULES: list[tuple[re.Pattern[str], str, object]] = [
    ...
    (
        re.compile(r"security_events|Dependabot API|gh auth refresh", re.I),
        "human",
        lambda r, c: (
            AttemptResult(0, "security-scope", "Dependabot API readable", False)
            if run_cmd(r, bash_script(r, "scripts/ensure-gh-security-scope.sh", "--refresh"))[0] == 0
            and run_cmd(r, bash_script(r, "scripts/ensure-gh-security-scope.sh", "--strict"))[0] == 0
            else AttemptResult(
                1,
                "security-scope",
                "Run bash scripts/automate-human-backlog.sh --refresh-gh",
                True,
            )
        ),
    ),
    (
        re.compile(r"stale\.yml|weekly-health|dependabot-automerge|HUMAN_BACKLOG|Align-0\.15", re.I),
        "human",
        lambda r, c: (
            AttemptResult(0, "human-backlog", "automate-human-backlog.sh completed", False)
            if run_cmd(r, bash_script(r, "scripts/automate-human-backlog.sh"))[0] == 0
            else AttemptResult(1, "human-backlog", "automate-human-backlog.sh failed", True)
        ),
    ),
]
