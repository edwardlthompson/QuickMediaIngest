...
**Platform/Tech Stack:** .NET 8 WPF (C#), xUnit, WiX MSI, GitHub Releases on Windows

**Purpose & Goals:** Quick Media Ingest — fast FOSS media importer for photographers. Ingest photos/videos from SD cards, local drives, and FTP into dated shoot-based folders with configurable naming, metadata, and safety checks. Goals: reliable ingest, photographer-friendly UX, pure FOSS MIT distribution. Non-goals: cloud sync, proprietary SDKs, telemetry.
