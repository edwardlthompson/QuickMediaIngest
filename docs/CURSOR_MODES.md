...
| New feature container, ADR, parallel scope | **Plan** | New `MainViewModel.*.partial.cs` domain split |
| Same fix failed 3× or CI red, unknown cause | **Debug** | FTP thumbnail tier regression; flaky `dotnet test` on CI |
| Mid-task architecture pivot | **Plan** | Shared Core type change during ingest pipeline work |
